#!/bin/bash
# Keep original stdout/stderr so selected steps can opt out of global quiet mode.
exec 3>&1 4>&2
exec >/dev/null 2>&1
...
pkg_install() {
    ...
    case "$pkg_manager" in
        apt-get|apt)
            _sudo "$pkg_manager" update 1>&3 2>&4
...
print_path_refresh_hint() {
    ...
    if [ ${#PATH_SOURCED_FILES[@]} -gt 0 ]; then
        echo "已自动 source 以下 shell 配置使当前会话立即生效："
        printf ' - %s\n' "${PATH_SOURCED_FILES[@]}"
    elif [ ${#PATH_PERSIST_FILES[@]} -gt 0 ]; then
...
source_shell_profiles() {
    ...
    if [ $source_rc -ne 0 ]; then
        echo "WARN: 某些 shell 配置文件 source 失败，但安装已完成。" >&2
        FAILED_STEPS+=("source shell 配置 (partial-failed)")
    fi
...
echo "安装完成！"
run_step "刷新 shell 配置使当前会话立即生效" source_shell_profiles
print_path_refresh_hint
